(function () {
  ...
  function renderWishlist(payload) {
    ...
    grid.innerHTML = items.map(function (item) {
      ...
      const priceMarkup = item.is_on_sale
        ? '<p class="gf-account-saved-price"><strong>' + escapeHtml(item.sale_price_label || item.price_label) + '</strong> <span style="text-decoration:line-through;color:#8a7c67;font-weight:500;">' + escapeHtml(item.original_price_label || '') + '</span></p><p class="gf-account-note" style="color:#b4374e;font-weight:700;">' + escapeHtml(item.sale_badge || 'SALE') + (item.sale_caption ? ' · ' + escapeHtml(item.sale_caption) : '') + (item.display_discount_percent ? ' · ' + escapeHtml(String(item.display_discount_percent)) + '% OFF' : '') + '</p>'
        : '<p class="gf-account-saved-price">' + escapeHtml(item.price_label) + '</p>';
      return '<article class="gf-account-saved-card">' + image + '<div class="gf-account-saved-body"><div class="gf-account-saved-top"><h4>' + escapeHtml(item.name) + '</h4><span class="gf-account-stock-badge ' + escapeAttribute(item.stock_class) + '">' + escapeHtml(item.stock_label) + '</span></div>' + priceMarkup + meta + '<div class="gf-account-saved-actions">' + viewAction + '<button type="button" class="gf-account-btn gf-account-btn-primary" data-gf-wishlist-action="cart" data-gf-wishlist-source="' + escapeAttribute(item.source) + '" data-gf-wishlist-id="' + escapeAttribute(item.id) + '" data-gf-wishlist-raw-id="' + escapeAttribute(item.raw_id || item.id) + '" data-gf-sku="' + escapeAttribute(item.sku) + '" data-gf-name="' + escapeAttribute(item.name) + '" data-gf-price="' + escapeAttribute(item.price) + '" data-gf-image="' + escapeAttribute(item.image) + '" data-gf-size="' + escapeAttribute(item.size) + '" data-gf-color="' + escapeAttribute(item.color) + '"' + (item.can_add_to_cart ? '' : ' disabled') + '>Add to Cart</button><button type="button" class="gf-account-btn gf-account-btn-secondary" data-gf-wishlist-action="remove" data-gf-wishlist-source="' + escapeAttribute(item.source) + '" data-gf-wishlist-id="' + escapeAttribute(item.id) + '" data-gf-wishlist-raw-id="' + escapeAttribute(item.raw_id || item.id) + '">Remove</button></div></div></article>';
    }).join('');
  }
