document.addEventListener("DOMContentLoaded", function () {
  ...
  const localeHelper = window.GFCategoryLocaleHelper ? window.GFCategoryLocaleHelper.create("home-living") : null;
  ...
  const baseProducts = [
    { title: "Cushion Cover", priceEur: 49, folderTemplate: "Cart/products/tshirt-men/Men france/{color}/" },
    { title: "Ceramic Mug", priceEur: 39, folderTemplate: "Cart/products/tshirt-men/men italy/{color}/" },
    { title: "Coasters Set", priceEur: 29, folderTemplate: "Cart/products/tshirt-men/men usa/{color}/" },
    { title: "Wall Art Poster", priceEur: 59, folderTemplate: "Cart/products/tshirt-women/women france/{color}/" },
    { title: "Soft Blanket", priceEur: 69, folderTemplate: "Cart/products/tshirt-women/Women italy/{color}/" },
    { title: "Decor Candle", priceEur: 34, folderTemplate: "Cart/products/tshirt-women/Women japon/{color}/" }
  ];
  ...
  function getCartItems() { return safeReadArray(CART_KEY); }
  function getWishlistItems() { return safeReadArray(WISHLIST_KEY); }
  ...
  function sectionName(section) {
    ...
    if (fixedName) {
      const titleNode = section.querySelector(".gx25-men-title, .gx25-home-title");
      if (titleNode) titleNode.textContent = localeHelper ? localeHelper.translateSection(fixedName) : fixedName;
      return fixedName;
    }

    const titleNode = section.querySelector(".gx25-men-title, .gx25-home-title");
    return (titleNode ? titleNode.textContent : "Home Living").trim();
  }

  function buildImages(folderTemplate, colorCode) {
    const folder = folderTemplate.replace("{color}", colorCode);
    return [folder + "400.jpg", folder + "400-1.jpg", folder + "400-2.jpg", folder + "400-3.jpg"];
  }

  function buildProductsForSection(name) {
    return baseProducts.map(function (base, index) {
      return {
        id: "home-living-" + name.toLowerCase().replace(/\s+/g, "-") + "-" + (index + 1),
        ...
        price: localeHelper ? localeHelper.formatPrice(base.priceEur) : (base.priceEur.toFixed(2).replace('.', ',') + ' €'),
  ...
  function createColorDots(product) {
    return product.colors.map(function (code, idx) {
  ...
  function wishlistHas(id) {
    return getWishlistItems().some(function (item) { return item.id === id; });
  }
  ...
  async function addCart(item) {
    ...
    const idx = list.findIndex(function (x) { return x.id === item.id && x.color === item.color; });
    if (idx >= 0) {
  ...
  function initSection(section) {
    ...
    function cards() { return track.querySelectorAll(".gx25-card"); }
    ...
    cardNodes.forEach(function (card, idx) {
      ...
      favBtn.addEventListener("click", function () {
        ...
        const active = toggleWishlist({ id: product.id, title: product.title, price: pricing.priceText, priceNumber: pricing.priceNumber, image: imageSet[0] });
      ...
      inPrev.addEventListener("click", function (event) {
        ...
        setTimeout(function () { img.src = imageSet[imageIndex]; img.style.opacity = "1"; }, 120);
      });
      ...
      colorDots.forEach(function (dot) {
        dot.addEventListener("click", function () {
          ...
          imageIndex = 0;
          img.style.opacity = "0.35";
          setTimeout(function () { img.src = imageSet[0]; img.style.opacity = "1"; }, 120);
        });
      });
