document.addEventListener("DOMContentLoaded", function () {
  ...
  const localeHelper = window.GFCategoryLocaleHelper ? window.GFCategoryLocaleHelper.create("accessories") : null;
  ...
  function sectionName(section) {
    ...
    if (fixedName) {
      const titleNode = section.querySelector(".gx25-men-title, .gx25-accessories-title");
      if (titleNode) titleNode.textContent = localeHelper ? localeHelper.translateSection(fixedName) : fixedName;
      return fixedName;
    }

    const titleNode = section.querySelector(".gx25-men-title, .gx25-accessories-title");
    return (titleNode ? titleNode.textContent : "Accessories").trim();
  }
  ...
  function buildProductsForSection(name) {
    return baseProducts.map(function (base, index) {
      return {
        id: "accessories-" + name.toLowerCase().replace(/\s+/g, "-") + "-" + (index + 1),
