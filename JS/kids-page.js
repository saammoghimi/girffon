document.addEventListener("DOMContentLoaded", function () {
  ...
  function sectionName(section) {
    ...
    if (fixedName) {
      const titleNode = section.querySelector(".gx25-men-title, .gx25-kids-title");
      if (titleNode) titleNode.textContent = localeHelper ? localeHelper.translateSection(fixedName) : fixedName;
      return fixedName;
    }

    const titleNode = section.querySelector(".gx25-men-title, .gx25-kids-title");
    return (titleNode ? titleNode.textContent : "Kids").trim();
  }
  ...
  function buildProductsForSection(name) {
    return baseProducts.map(function (base, index) {
      return {
        id: "kids-" + name.toLowerCase().replace(/\s+/g, "-") + "-" + (index + 1),
